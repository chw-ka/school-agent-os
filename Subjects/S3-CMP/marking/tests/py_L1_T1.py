...
# Model answer for heart pattern
MODEL_HEART = """ **   ** 
*********
 *******
  *****  
...
def run_heart_pattern(student_code):
...
def analyze_code_quality(code):
    ...
    # Check for heart-related symbols
    heart_symbols = ['*']
    found_symbols = [symbol for symbol in heart_symbols if symbol in code]
    if found_symbols:
        score += 1
        explanation.append(f"✅ 包含圖案符號: {', '.join(found_symbols)}")
...
def evaluate_heart_pattern(filepath):
    try:
    ...
    result = run_heart_pattern(student_code)
    ...
    if result.get("runs"):
        ...
        similarity = calculate_similarity(student_output, MODEL_HEART)
        ...
        if similarity >= 0.95:  # Almost identical
            ...
            # Check if it's still a reasonable heart pattern attempt
...
def test(submissions):
    for idx, row in submissions.iterrows():
        ...
        # (10 marks) The python code creates a heart pattern
        section_description = "Python程式碼輸出心型圖案"
        filepath = row["filepath"]
        section_mark, remarks = evaluate_heart_pattern(filepath)
