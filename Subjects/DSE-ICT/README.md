...
- `question-bank/`：結構化題目 JSON（**出卷參考時讀這裡，毋須每次 OCR**）
  ...
  - `concept_map.json`（tree）：C&A + bank 統計 — `build_concept_map.py`
  - `style_patterns.json`：問法／語法／專有名詞 patterns（`extract_style_patterns.py` 生成；唔係題目原文）
  - `index.json`：各年份／試卷目錄
- `edb/`：教育局課程及評估指引等官方文件
...
結構化 JSON 供 **style_patterns** / **concept_map** 統計（見 `shared-tools/paper-generator/F5_ICT_CONCEPT_GENERATE_FLOW.md`）。
