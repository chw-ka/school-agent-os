...
FROM tblAssessment2 a 
INNER JOIN tblAssessmentClass2 ac ON a.idAssessment = ac.idAssessment 
INNER JOIN tblClass c ON ac.class = c.class 
INNER JOIN vwStaffSubject ss ON ss.class = ac.class AND a.idSubject = ss.idSubject and ss.flgTeach = 1
LEFT JOIN ( 
SELECT distinct sa.idAssessment, ss.class 
FROM tblStudentAssessment2 sa 
INNER JOIN tblAssessment2 a ON a.idAssessment = sa.idAssessment 
INNER JOIN vwStudentSubject ss ON sa.idStudent = ss.idStudent AND ss.idSubject = a.idSubject 
WHERE sa.score is NOT null OR sa.flgAchieved = 1
) r ON a.idAssessment = r.idAssessment AND ac.class = r.class 
WHERE r.idAssessment is null -- AND c.form IN (1, 2, 3) 
ORDER BY ss.idStaff, ac.class, a.idSubject

--Check if undesired value found in field "test" in tblStudentPaperScore
SELECT distinct ss.idStaff, ss.class, ss.idSubject 
FROM tblStudentAssessment2 sa 
INNER JOIN tblAssessment2 a ON sa.idAssessment = a.idAssessment 
...
INNER JOIN tblFormTerm ft ON sp.form = ft.form AND (a.dateFrom between ft.dateStart AND ft.dateEnd) 
...
INNER JOIN tblFormTerm ft ON c.form = ft.form AND (a.dateFrom between ft.dateStart AND ft.dateEnd) 
WHERE ((weight_test_1 is null AND ft.term = 1) OR (weight_test_2 is null AND ft.term = 2)) 
