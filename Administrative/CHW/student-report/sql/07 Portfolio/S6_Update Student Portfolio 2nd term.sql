...
-- 1a 下學期總分    (score 要 3 位數, 因 display 有 1 個 d.p. )
	...
	WHERE idPaper = '' and flgStandard = 0 and section = 'O' and term = 2 and form in (6)
...
-- 1b 全年總成績    (score 要 3 位數, 因 display 有 1 個 d.p. )
	...
	FROM tblZStudentRank2
	WHERE idPaper = '' and flgStandard = 0 and section = 'O' and term = 0 and form in (6)
...
insert tblYComment
select 2024, idComment, idCommentGroup, comment from tblYComment
where yearSchool = 2021
...
--7.0 paper  
	...
	SELECT form, idPaper
	FROM vwStudentPaper 
	WHERE (flgTerm1 = 1 or flgTerm2 = 1) AND form IN (6) 
	UNION 
	SELECT form, idSubject
	FROM vwStudentPaper 
	WHERE (flgTerm1 = 1 or flgTerm2 = 1) AND idPaper <> idSubject AND form IN (6)
	) r 
	...
	FROM vwStudentPaper 
	WHERE flgTerm1 = 1 AND form IN (6) 
	UNION 
	SELECT idStudent, 2024, form, idSubject, 1
	FROM vwStudentPaper 
	WHERE flgTerm1 = 1 AND idPaper <> idSubject AND form IN (6)
...
-- 若真的要做，可能要先清 tblYStudentSubjectPost, tblYStudentPaperScore, 如下
	...
	FROM vwStudentPaper 
	WHERE flgTerm2 = 1 AND form IN (6) 
	UNION 
	SELECT idStudent, 2024, form, idSubject, 2
	FROM vwStudentPaper 
	WHERE flgTerm2 = 1 AND idPaper <> idSubject AND form IN (6)
...
--delete from tblYStudentSubjectAttitude where yearSchool = 2012
	...
	--Select 2020, idComment, nameChinese, nameEnglish from tblAttitudeComment
...
-- 10 分數 StudentPaperScore  (score 要 2 位數) (2014-09-12 Log  Not Success from here)
	...
	WHERE zsr.term in (2) and zsr.form in (6)
	UNION 
	...
	where yspt.form in (6) and grade_exam_2 is NOT null
	...
	FROM tblZStudentRank2 zsr 
	INNER JOIN tblYStudentPaperTerm yspt ON zsr.idStudent = yspt.idStudent AND zsr.idPaper = yspt.idPaper AND yspt.yearSchool = 2024 AND yspt.term = zsr.term and zsr.section = 'O' and zsr.flgStandard = 0 
	WHERE zsr.term in (0) and zsr.form in (6)
	UNION 
