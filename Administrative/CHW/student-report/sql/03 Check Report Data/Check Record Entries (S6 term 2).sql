...
AND sp.form in (6) AND sp.flgTerm2 = 1 and sps.flgIgnore_2 = 0 and sps.flgAbsent_2 = 0
...
INNER JOIN tblPaper p ON sp.idPaper = sps.idPaper AND p.formGroup = sp.formGroup 
INNER JOIN vwStaffSubject ss ON sp.idSubject = ss.idSubject AND sp.class = ss.class AND ss.flgTeach = 1
where sps.idPaper in ('BBS','PED','MUS') and sps.grade_exam_2 is NULL and sp.form in (6) and sp.flgTerm2 = 1 and sps.flgIgnore_2 = 0 and sps.flgAbsent_2 = 0
order by ss.idStaff, sp.class, sps.idPaper, s.class, s.numberClass
...
AND sp.form in (6) AND sp.flgTerm1 = 1 and sps.flgIgnore_2 = 0
ORDER BY ss.idStaff, sp.class, sps.idPaper, s.class, s.numberClass
...
WHERE s.form in (6) and s.flgTerm1 = 1 and ss.flgTerm2 = 1 and 
	(
		lesson_2 is null 
		OR 
		(
