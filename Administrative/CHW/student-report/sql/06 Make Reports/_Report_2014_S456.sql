...
USE [db13_14]
GO
/****** 物件:  StoredProcedure [dbo].[stpGenerateReport_S456]    指令碼日期: 13/4/2012 11:14:57 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[stpGenerateReport_S456_2014]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[stpGenerateReport_S456_2014]
GO

CREATE PROCEDURE [dbo].[stpGenerateReport_S456_2014]
...
--create Demerit table

if exists (select * from tempdb..sysobjects where name = '##tblDemerit')
drop table [tempdb].[dbo].[##tblDemerit]

CREATE TABLE [dbo].[##tblDemerit] (
	[idStudent] [int] NOT NULL,
	[Demerit01] [nvarchar] (40) COLLATE Chinese_Taiwan_Stroke_CI_AS NOT NULL,
...
if exists (select * from tempdb..sysobjects where name = '##tblMeritRow')
drop table [tempdb].[dbo].[##tblMeritRow]

CREATE TABLE [dbo].[##tblMeritRow] (
	[idRow] [smallint] IDENTITY (1, 1) NOT NULL,
	[idStudent] [int] NOT NULL,
	[row] [smallint] NOT NULL,
	[Merit] [nvarchar] (80) COLLATE Chinese_Taiwan_Stroke_CI_AS NOT NULL
) ON [PRIMARY]

if exists (select * from tempdb..sysobjects where name = '##tblMerit')
drop table [tempdb].[dbo].[##tblMerit]

CREATE TABLE [dbo].[##tblMerit] (
	[idStudent] [int] NOT NULL,
--	[flgNewLine] [bit] NOT NULL,
	[Merit01] [nvarchar] (200) COLLATE Chinese_Taiwan_Stroke_CI_AS NOT NULL,
--	[Merit02] [nvarchar] (20) COLLATE Chinese_Taiwan_Stroke_CI_AS NOT NULL,
--	[Merit03] [nvarchar] (20) COLLATE Chinese_Taiwan_Stroke_CI_AS NOT NULL,
--	[Merit04] [nvarchar] (20) COLLATE Chinese_Taiwan_Stroke_CI_AS NOT NULL
) ON [PRIMARY]
...
UPDATE ##tblResultRow
set Sbj_1 = ''
WHERE idSubject in ('BBS') and 
	  idStudent in (select idStudent from tblStudent where left(class,1) in (4,5))
...
WHERE zsr.score < 4950 AND zsr.form = @form AND zsr.flgStandard = 0 AND zsr.term = @term
UNION
...
CASE WHEN totalDS > 0 THEN
	...
	CASE WHEN smallDS > 0 THEN 
		Case when largeDS > 0 then char(13) + char(13) Else N' ' End + 
        convert(nvarchar(5), smallDS) + N' 小過' ELSE '' END +
	CASE WHEN demeritDS > 0 THEN 
		Case when ( largeDS > 0 and  smallDS > 0 ) or  ( largeDS = 0 and  smallDS = 0 )   then  N' ' Else char(13) + char(10) End + 
		convert(nvarchar(5), demeritDS) + N' 缺點' ELSE '' END 
	ELSE ''END
+
	CASE WHEN totalHW > 0 THEN
		CASE WHEN totalDS > 0 THEN char(13) + char(13) + char(10)  ELSE '' END + N'因交功課問題記' +
		CASE WHEN largeHW > 0 THEN N' ' + convert(nvarchar(5), largeHW) + N' 大過' ELSE '' END +
		CASE WHEN smallHW > 0 THEN 
			Case when largeHW > 0 then char(13) + char(13) Else N' ' End + 
			convert(nvarchar(5), smallHW) + N' 小過' ELSE '' END +
		CASE WHEN demeritHW > 0 THEN 
			Case when ( largeHW > 0 and  smallHW > 0 ) or  ( largeHW = 0 and  smallHW = 0 )   then  N' ' Else char(13) + char(10) End + 
			convert(nvarchar(5), demeritHW) + N' 缺點' ELSE '' END
...
insert ##tblMerit (idstudent, merit01)
SELECT distinct idStudent,
CASE WHEN total1 > 0 THEN
	N'德行表現優異' + char(13) + char(10) + '獲' +
...
CASE WHEN total2 > 0 THEN
	CASE WHEN total1 > 0 THEN  char(13) + char(13) + char(10) ELSE '' END + 
	N'出外參賽為校爭光' + char(13) + char(10) + '獲' +
...
CASE WHEN total3 > 0 THEN
	CASE WHEN total1 + total2 > 0 THEN  char(13) + char(13) + char(10) ELSE '' END + 
	N'校內服務表現優異' + char(13) + char(10) + '獲' +
...
CASE WHEN total4 > 0 THEN
	CASE WHEN total1 + total2 + total3 > 0 THEN char(13) + char(13) + char(10) ELSE '' END + 
	N'校外服務表現優異' + char(13) + char(10) + '獲' +
...
--Master
INSERT tblZStudentReport_S456 (term, Class, Num, form, idStudent, [Year], Term_CName, Term_EName, CName, EName, Gender, StuID, CTeacher)
...
--Result
UPDATE tblZStudentReport_S456 SET Sbj01_EName = Sbj_EName, Sbj01_CName = Sbj_CName, Sbj01_1 = Sbj_1, Sbj01_2 = Sbj_2, Sbj01_3 = Sbj_3, Lsn_01 = Lsn, Asm_01 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 1
UPDATE tblZStudentReport_S456 SET Sbj02_EName = Sbj_EName, Sbj02_CName = Sbj_CName, Sbj02_1 = Sbj_1, Sbj02_2 = Sbj_2, Sbj02_3 = Sbj_3, Lsn_02 = Lsn, Asm_02 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 2
UPDATE tblZStudentReport_S456 SET Sbj03_EName = Sbj_EName, Sbj03_CName = Sbj_CName, Sbj03_1 = Sbj_1, Sbj03_2 = Sbj_2, Sbj03_3 = Sbj_3, Lsn_03 = Lsn, Asm_03 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 3
UPDATE tblZStudentReport_S456 SET Sbj04_EName = Sbj_EName, Sbj04_CName = Sbj_CName, Sbj04_1 = Sbj_1, Sbj04_2 = Sbj_2, Sbj04_3 = Sbj_3, Lsn_04 = Lsn, Asm_04 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 4
UPDATE tblZStudentReport_S456 SET Sbj05_EName = Sbj_EName, Sbj05_CName = Sbj_CName, Sbj05_1 = Sbj_1, Sbj05_2 = Sbj_2, Sbj05_3 = Sbj_3, Lsn_05 = Lsn, Asm_05 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 5
UPDATE tblZStudentReport_S456 SET Sbj06_EName = Sbj_EName, Sbj06_CName = Sbj_CName, Sbj06_1 = Sbj_1, Sbj06_2 = Sbj_2, Sbj06_3 = Sbj_3, Lsn_06 = Lsn, Asm_06 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 6
UPDATE tblZStudentReport_S456 SET Sbj07_EName = Sbj_EName, Sbj07_CName = Sbj_CName, Sbj07_1 = Sbj_1, Sbj07_2 = Sbj_2, Sbj07_3 = Sbj_3, Lsn_07 = Lsn, Asm_07 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 7
UPDATE tblZStudentReport_S456 SET Sbj08_EName = Sbj_EName, Sbj08_CName = Sbj_CName, Sbj08_1 = Sbj_1, Sbj08_2 = Sbj_2, Sbj08_3 = Sbj_3, Lsn_08 = Lsn, Asm_08 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 8
UPDATE tblZStudentReport_S456 SET Sbj09_EName = Sbj_EName, Sbj09_CName = Sbj_CName, Sbj09_1 = Sbj_1, Sbj09_2 = Sbj_2, Sbj09_3 = Sbj_3, Lsn_09 = Lsn, Asm_09 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 9
UPDATE tblZStudentReport_S456 SET Sbj10_EName = Sbj_EName, Sbj10_CName = Sbj_CName, Sbj10_1 = Sbj_1, Sbj10_2 = Sbj_2, Sbj10_3 = Sbj_3, Lsn_10 = Lsn, Asm_10 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 10
UPDATE tblZStudentReport_S456 SET Sbj11_EName = Sbj_EName, Sbj11_CName = Sbj_CName, Sbj11_1 = Sbj_1, Sbj11_2 = Sbj_2, Sbj11_3 = Sbj_3, Lsn_11 = Lsn, Asm_11 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 11
UPDATE tblZStudentReport_S456 SET Sbj12_EName = Sbj_EName, Sbj12_CName = Sbj_CName, Sbj12_1 = Sbj_1, Sbj12_2 = Sbj_2, Sbj12_3 = Sbj_3, Lsn_12 = Lsn, Asm_12 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 12
UPDATE tblZStudentReport_S456 SET Sbj13_EName = Sbj_EName, Sbj13_CName = Sbj_CName, Sbj13_1 = Sbj_1, Sbj13_2 = Sbj_2, Sbj13_3 = Sbj_3, Lsn_13 = Lsn, Asm_13 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 13
UPDATE tblZStudentReport_S456 SET Sbj14_EName = Sbj_EName, Sbj14_CName = Sbj_CName, Sbj14_1 = Sbj_1, Sbj14_2 = Sbj_2, Sbj14_3 = Sbj_3, Lsn_14 = Lsn, Asm_14 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 14
UPDATE tblZStudentReport_S456 SET Sbj15_EName = Sbj_EName, Sbj15_CName = Sbj_CName, Sbj15_1 = Sbj_1, Sbj15_2 = Sbj_2, Sbj15_3 = Sbj_3, Lsn_15 = Lsn, Asm_15 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 15
UPDATE tblZStudentReport_S456 SET Sbj16_EName = Sbj_EName, Sbj16_CName = Sbj_CName, Sbj16_1 = Sbj_1, Sbj16_2 = Sbj_2, Sbj16_3 = Sbj_3, Lsn_16 = Lsn, Asm_16 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 16
UPDATE tblZStudentReport_S456 SET Sbj17_EName = Sbj_EName, Sbj17_CName = Sbj_CName, Sbj17_1 = Sbj_1, Sbj17_2 = Sbj_2, Sbj17_3 = Sbj_3, Lsn_17 = Lsn, Asm_17 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 17
UPDATE tblZStudentReport_S456 SET Sbj18_EName = Sbj_EName, Sbj18_CName = Sbj_CName, Sbj18_1 = Sbj_1, Sbj18_2 = Sbj_2, Sbj18_3 = Sbj_3, Lsn_18 = Lsn, Asm_18 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 18
UPDATE tblZStudentReport_S456 SET Sbj19_EName = Sbj_EName, Sbj19_CName = Sbj_CName, Sbj19_1 = Sbj_1, Sbj19_2 = Sbj_2, Sbj19_3 = Sbj_3, Lsn_19 = Lsn, Asm_19 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 19
UPDATE tblZStudentReport_S456 SET Sbj20_EName = Sbj_EName, Sbj20_CName = Sbj_CName, Sbj20_1 = Sbj_1, Sbj20_2 = Sbj_2, Sbj20_3 = Sbj_3, Lsn_20 = Lsn, Asm_20 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 20
UPDATE tblZStudentReport_S456 SET Sbj21_EName = Sbj_EName, Sbj21_CName = Sbj_CName, Sbj21_1 = Sbj_1, Sbj21_2 = Sbj_2, Sbj21_3 = Sbj_3, Lsn_21 = Lsn, Asm_21 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 21
UPDATE tblZStudentReport_S456 SET Sbj22_EName = Sbj_EName, Sbj22_CName = Sbj_CName, Sbj22_1 = Sbj_1, Sbj22_2 = Sbj_2, Sbj22_3 = Sbj_3, Lsn_22 = Lsn, Asm_22 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 22
UPDATE tblZStudentReport_S456 SET Sbj23_EName = Sbj_EName, Sbj23_CName = Sbj_CName, Sbj23_1 = Sbj_1, Sbj23_2 = Sbj_2, Sbj23_3 = Sbj_3, Lsn_23 = Lsn, Asm_23 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 23
UPDATE tblZStudentReport_S456 SET Sbj24_EName = Sbj_EName, Sbj24_CName = Sbj_CName, Sbj24_1 = Sbj_1, Sbj24_2 = Sbj_2, Sbj24_3 = Sbj_3, Lsn_24 = Lsn, Asm_24 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 24
UPDATE tblZStudentReport_S456 SET Sbj25_EName = Sbj_EName, Sbj25_CName = Sbj_CName, Sbj25_1 = Sbj_1, Sbj25_2 = Sbj_2, Sbj25_3 = Sbj_3, Lsn_25 = Lsn, Asm_25 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 25
UPDATE tblZStudentReport_S456 SET Sbj26_EName = Sbj_EName, Sbj26_CName = Sbj_CName, Sbj26_1 = Sbj_1, Sbj26_2 = Sbj_2, Sbj26_3 = Sbj_3, Lsn_26 = Lsn, Asm_26 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 26
UPDATE tblZStudentReport_S456 SET Sbj27_EName = Sbj_EName, Sbj27_CName = Sbj_CName, Sbj27_1 = Sbj_1, Sbj27_2 = Sbj_2, Sbj27_3 = Sbj_3, Lsn_27 = Lsn, Asm_27 = Asm FROM tblZStudentReport_S456 zsr, ##tblResultRow rr WHERE term = @term AND form = @form AND zsr.idStudent = rr.idStudent AND rr.row = 27
...
IF @term = 1
BEGIN
	UPDATE tblZStudentReport_S456 SET Cnd01 = conduct_1_1 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_1_1 is not null
	UPDATE tblZStudentReport_S456 SET Cnd02 = conduct_2_1 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_2_1 is not null
	UPDATE tblZStudentReport_S456 SET Cnd03 = conduct_3_1 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_3_1 is not null
	UPDATE tblZStudentReport_S456 SET Cnd04 = conduct_4_1 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_4_1 is not null
	UPDATE tblZStudentReport_S456 SET Cnd05 = conduct_5_1 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_5_1 is not null
END
ELSE
BEGIN
	UPDATE tblZStudentReport_S456 SET Cnd01 = conduct_1_2 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_1_2 is not null
	UPDATE tblZStudentReport_S456 SET Cnd02 = conduct_2_2 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_2_2 is not null
	UPDATE tblZStudentReport_S456 SET Cnd03 = conduct_3_2 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_3_2 is not null
	UPDATE tblZStudentReport_S456 SET Cnd04 = conduct_4_2 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_4_2 is not null
	UPDATE tblZStudentReport_S456 SET Cnd05 = conduct_5_2 FROM tblZStudentReport_S456 zsr, tblStudentConduct sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND conduct_5_2 is not null
END

--comment
IF @term = 1
BEGIN
	UPDATE tblZStudentReport_S456 SET Cmt01 = custom_1_1 FROM tblZStudentReport_S456 zsr, tblStudentComment sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND custom_1_1 is not null
	UPDATE tblZStudentReport_S456 SET Cmt02 = custom_2_1 FROM tblZStudentReport_S456 zsr, tblStudentComment sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND custom_2_1 is not null
	UPDATE tblZStudentReport_S456 SET Cmt03 = custom_3_1 FROM tblZStudentReport_S456 zsr, tblStudentComment sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND custom_3_1 is not null
	UPDATE tblZStudentReport_S456 SET Cmt04 = custom_4_1 FROM tblZStudentReport_S456 zsr, tblStudentComment sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND custom_4_1 is not null

	UPDATE tblZStudentReport_S456 SET Cmt01 = comment FROM tblZStudentReport_S456 zsr, tblStudentComment sc, tblComment c WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND sc.comment_1_1 = c.idComment AND sc.comment_1_1 is not null
	UPDATE tblZStudentReport_S456 SET Cmt02 = comment FROM tblZStudentReport_S456 zsr, tblStudentComment sc, tblComment c WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND sc.comment_2_1 = c.idComment AND sc.comment_2_1 is not null
	UPDATE tblZStudentReport_S456 SET Cmt03 = comment FROM tblZStudentReport_S456 zsr, tblStudentComment sc, tblComment c WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND sc.comment_3_1 = c.idComment AND sc.comment_3_1 is not null
	UPDATE tblZStudentReport_S456 SET Cmt04 = comment FROM tblZStudentReport_S456 zsr, tblStudentComment sc, tblComment c WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND sc.comment_4_1 = c.idComment AND sc.comment_4_1 is not null
END
ELSE
BEGIN
	UPDATE tblZStudentReport_S456 SET Cmt01 = custom_1_2 FROM tblZStudentReport_S456 zsr, tblStudentComment sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND custom_1_2 is not null
	UPDATE tblZStudentReport_S456 SET Cmt02 = custom_2_2 FROM tblZStudentReport_S456 zsr, tblStudentComment sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND custom_2_2 is not null
	UPDATE tblZStudentReport_S456 SET Cmt03 = custom_3_2 FROM tblZStudentReport_S456 zsr, tblStudentComment sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND custom_3_2 is not null
	UPDATE tblZStudentReport_S456 SET Cmt04 = custom_4_2 FROM tblZStudentReport_S456 zsr, tblStudentComment sc WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND custom_4_2 is not null

	UPDATE tblZStudentReport_S456 SET Cmt01 = comment FROM tblZStudentReport_S456 zsr, tblStudentComment sc, tblComment c WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND sc.comment_1_2 = c.idComment AND sc.comment_1_2 is not null
	UPDATE tblZStudentReport_S456 SET Cmt02 = comment FROM tblZStudentReport_S456 zsr, tblStudentComment sc, tblComment c WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND sc.comment_2_2 = c.idComment AND sc.comment_2_2 is not null
	UPDATE tblZStudentReport_S456 SET Cmt03 = comment FROM tblZStudentReport_S456 zsr, tblStudentComment sc, tblComment c WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND sc.comment_3_2 = c.idComment AND sc.comment_3_2 is not null
	UPDATE tblZStudentReport_S456 SET Cmt04 = comment FROM tblZStudentReport_S456 zsr, tblStudentComment sc, tblComment c WHERE term = @term AND form = @form AND zsr.idStudent = sc.idStudent AND sc.comment_4_2 = c.idComment AND sc.comment_4_2 is not null
END


--Demerit
Update tblZStudentReport_S456
SET Demerit = Demerit01
From tblZStudentReport_S456 zsr
Inner Join ##tblDemerit d on zsr.idstudent = d.idstudent
...
SET Merit = merit01
From tblZStudentReport_S456 zsr
Inner Join ##tblMerit m on zsr.idstudent = m.idstudent
where zsr.term = @term AND zsr.form = @form
...
-- Term 1
stpGenerateReport_S456_2014 4, 1
Go

stpGenerateReport_S456_2014 5, 1
Go

-- Term 1 處理網考學生成績表，刪除平均分、班名次及級名次

--select * from tblZStudentReport_S456
--where term = 1 and idStudent in (
--	select s.idStudent from tblStudentReportRemark a
--	left join tblStudent s on s.idStudent = a.idStudent
--	where a.term = 1 and a.nameChinese like '受%' and left(s.class,1) in (4,5)
--)
...
-- 處理 CHI, ENG 缺席
select * from tblZStudentReport_S456
where Term_EName = 1 and ( Sbj02_2 = '　　缺席' and Sbj03_2 = '　　缺席' and Sbj04_2 = '　　缺席' and Sbj05_2 = '　　缺席')
...
select * from tblZStudentReport_S456
where Term_EName = 1 and ( Sbj08_2 = '　　缺席' and Sbj09_2 = '　　缺席')
...
--*** 清走上課表現、功課考勤、總分、平均分、班名次、級名次 (From SW)

select * from tblZStudentReport_S456
where idStudent in (24071, 21071, 21047) and Term_EName = 1

update tblZStudentReport_S456
set ave3 = '--', CRank3 = '--', FRank3 = '--', Sbj01_3 = '', Sbj02_3 = '', Sbj03_3 = '', Sbj04_3 = '', Sbj05_3 = ''
, Sbj06_3 = '', Sbj07_3 = '', Sbj08_3 = '', Sbj09_3 = '', Sbj10_3 = '', Sbj11_3 = '', Sbj12_3 = '', Sbj13_3 = ''
, Sbj14_3 = '', Sbj15_3 = '', Sbj16_3 = '', Sbj17_3 = '', Sbj18_3 = '', Sbj19_3 = '', Sbj20_3 = '', Sbj21_3 = ''
, Sbj22_3 = '', Sbj23_3 = '', Sbj24_3 = '', Sbj25_3 = '', Sbj26_3 = '', Sbj27_3 = ''
where idStudent in (24071, 21071, 21047) and Term_EName = 1


-- Term 2
stpGenerateReport_S456_2014 4, 2
Go

stpGenerateReport_S456_2014 5, 2
Go

select * from tblZStudentReport_S456
where term = 2 and form = 4
...
-- 處理 CHI, ENG 缺席
select * from tblZStudentReport_S456
where Term_EName = 2 and ( Sbj02_2 = '　　缺席' and Sbj03_2 = '　　缺席' and Sbj04_2 = '　　缺席' and Sbj05_2 = '　　缺席')
...
select * from tblZStudentReport_S456
where Term_EName = 2 and ( Sbj08_2 = '　　缺席' and Sbj09_2 = '　　缺席')
...
select * from tblZStudentReport_S456
where idStudent in (20078) and Term_EName = 2
...
-- Term 2 處理網考學生成績表，刪除平均分、班名次及級名次

--select * from tblZStudentReport_S456
--where term = 2 and idStudent in (
--	select s.idStudent from tblStudentReportRemark a
--	left join tblStudent s on s.idStudent = a.idStudent
--	where a.term = 2 and a.nameChinese like '受%' and left(s.class,1) in (1,2,3,4,5)
--)
