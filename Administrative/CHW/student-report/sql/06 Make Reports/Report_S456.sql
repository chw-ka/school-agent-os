...
CREATE PROCEDURE [dbo].[stpGenerateReport_S456]
...
INNER JOIN (
	...
	CASE 
		...
		ELSE 
			CASE WHEN sp.idSubject = sp.idPaper THEN
				...
				THEN ''
				ELSE 
					CASE WHEN zsr1.score < 495 THEN N'(' + convert(nvarchar(6), convert(decimal(7,0), zsr1.score / 10.0)) + N')' ELSE  ' ' + convert(nvarchar(6), convert(decimal(7,0), zsr1.score / 10.0)) END
				END
			ELSE
				''
			END
	...
	CASE 
		...
		ELSE 
			...
			THEN ''
			ELSE 
				CASE WHEN zsr2.score < 495 THEN N'(' + convert(nvarchar(6), convert(decimal(7,0), zsr2.score / 10.0)) + N')' ELSE  ' ' + convert(nvarchar(6), convert(decimal(7,0), zsr2.score / 10.0)) END
			END
	END AS result_2,
	CASE 
		...
		ELSE
			CASE WHEN sp.idSubject = sp.idPaper THEN
				CASE 
					...
					ELSE 
						CASE WHEN zsr3.score < 495 THEN N'(' + convert(nvarchar(6), convert(decimal(7,0), zsr3.score / 10.0)) + N')' ELSE  ' ' + convert(nvarchar(6), convert(decimal(7,0), zsr3.score / 10.0)) END
				END
			ELSE
				''
			END
...
BEGIN
	...
	SET ave3 = '(' + convert(nvarchar(6), convert(decimal(7,1), zsr.score / 10.0)) + ')' 
...
stpGenerateReport_S456 4, 1
Go

stpGenerateReport_S456 5, 1
Go

stpGenerateReport_S456 6, 2
Go