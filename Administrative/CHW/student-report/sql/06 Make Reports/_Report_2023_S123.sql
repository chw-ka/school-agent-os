...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[stpGenerateReport_S123_2023]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[stpGenerateReport_S123_2023]
GO

CREATE PROCEDURE dbo.stpGenerateReport_S123_2023
...
-- Remarked by CM 2023_02_17  解決中三級科目過多問題
--UPDATE ##tblResultRow
--SET row = row + 1
--WHERE idSubject <> 'ENG'
...
-- Modified by CM 2023_02_17  解決初中加入只計算平時分 STEM 教育
UPDATE ##tblResultRow
set Sbj_1 = ''
WHERE idSubject in ('BBS', 'CMP', 'STM')
...
-- Modified by CM  2023_02_21  因個別學生承擔多項科長，而服務欄空間不足，故把科長轉往備註欄顯示
--UNION
...
-- Remarked by CM  2023_02_21  為解決過多科長及班職位未能於服務欄顯示(以下為原有程式碼)
...
-- Modified by CM 2023_02_21  以下為處理服務欄位置不足問題而修訂的程式碼，主要把科長之資料由服務欄轉往備註欄
INSERT ##tblECARow (idStudent, row, eca)
select s.idStudent, 0, r.srv
from vwStudent s
inner join
(SELECT s.idStudent, replace(u.nameChinese + CASE WHEN p.idPost <> 101 THEN p.nameChinese ELSE '' END + CASE WHEN ecac.nameChinese is not null THEN '(' + ecac.nameChinese + ')' ELSE '' END, ' ', '') AS srv, 1000 + p.keyOrder as src
FROM vwStudent s
INNER JOIN tblStudentUnitPost sup ON s.idStudent = sup.idStudent
INNER JOIN tblUnit u ON u.idUnit = sup.idUnit
INNER JOIN tblPost p ON p.idPost = sup.idPost
...
WHERE s.form = @form and idUnitGroup not in (7, 9)
UNION
SELECT idStudent, s.nameChinese + N'科' + CASE WHEN p.idPost <> 101 THEN p.nameChinese ELSE '' END + CASE WHEN ecac.nameChinese is not null THEN char(13) + char(10) + '(' + ecac.nameChinese + ')' ELSE '' END AS srv, 2000 + s.keyOrder as src
FROM tblStudentSubjectPost ssp
INNER JOIN tblSubject s ON ssp.idSubject = s.idSubject
INNER JOIN tblPost p ON p.idPost = ssp.idPost
LEFT JOIN tblECAComment ecac ON ecac.idComment = ssp.idComment AND @term = 2) as r on s.idStudent = r.idStudent
ORDER BY s.idStudent, r.src

-- Modification End 2023_02_21
...
--Term 1:
stpGenerateReport_S123_2014 1, 1
GO
stpGenerateReport_S123_2014 2, 1
GO
stpGenerateReport_S123_2023 3, 1
GO
stpGenerateReport_S123_2014 4, 1
GO
stpGenerateReport_S123_2014 5, 1
GO

-- Term 1 取消 STM 平時分 
select * from tblZStudentReport2
where form in (1,2) and Sbj20_1 <> '' and term = 1
...
--	where a.term = 1 and a.nameChinese like '受%' and left(s.class,1) in (1,2,3)
--)
...
-- 23-24 年度起中文科只留卷一及卷二，卷三至卷五則只於考試落 '--'
update tblZStudentReport2
set Sbj10_2 = '　　 --', Sbj11_2 = '　　 --'
where form in (1,2) and Term_EName = 1

update tblZStudentReport2
set Sbj09_2 = '　　 --', Sbj10_2 = '　　 --'
where form in (3) and Term_EName = 1

-- Term 1 處理中三科目過多，導致欄位 Ind （比重及平合格標示）不能顯示
--update tblZStudentReport2
--set Ind = '各科比重：英文300 中文300 數學300 物理100 化學100 生物100 中史200 地理100 生活與社會50 普通話50 聖經50 電腦認知50 STAEM教育50'
--where form = 3 and Term_EName = 1

--update tblZStudentReport2
--set Ind = '比重：英文、中文、數學300　物理、化學、生物、地理100　中史200　生社、普通話、聖經、電腦、STEAM 50' + char(13) + char(10) + N' ( ) 表示不及格'
--where form = 3 and Term_EName = 1
...
update tblZStudentReport2
set Ind = '各科比重：英文300 中文300 數學300 物理100 化學100 生物100 中史200 地理100 生活與社會50 普通話50 聖經50 電腦認知50' + char(13) + char(10) + 'STEAM教育50' + N'　( ) 表示不及格'
where form = 3 and idStudent in (
select idStudent from tblZStudentReport2
where Term_EName = 1 and form = 3 and (
      (charindex('(', Sbj01_1) > 0 or charindex('(', Sbj01_2) > 0 or charindex('(', Sbj01_3) > 0) or
      (charindex('(', Sbj02_1) > 0 or charindex('(', Sbj02_2) > 0 or charindex('(', Sbj02_3) > 0) or
      (charindex('(', Sbj03_1) > 0 or charindex('(', Sbj03_2) > 0 or charindex('(', Sbj03_3) > 0) or
      (charindex('(', Sbj04_1) > 0 or charindex('(', Sbj04_2) > 0 or charindex('(', Sbj04_3) > 0) or
      (charindex('(', Sbj05_1) > 0 or charindex('(', Sbj05_2) > 0 or charindex('(', Sbj05_3) > 0) or
      (charindex('(', Sbj06_1) > 0 or charindex('(', Sbj06_2) > 0 or charindex('(', Sbj06_3) > 0) or
      (charindex('(', Sbj07_1) > 0 or charindex('(', Sbj07_2) > 0 or charindex('(', Sbj07_3) > 0) or
      (charindex('(', Sbj08_1) > 0 or charindex('(', Sbj08_2) > 0 or charindex('(', Sbj08_3) > 0) or
      (charindex('(', Sbj09_1) > 0 or charindex('(', Sbj09_2) > 0 or charindex('(', Sbj09_3) > 0) or
      (charindex('(', Sbj10_1) > 0 or charindex('(', Sbj10_2) > 0 or charindex('(', Sbj10_3) > 0) or
      (charindex('(', Sbj11_1) > 0 or charindex('(', Sbj11_2) > 0 or charindex('(', Sbj11_3) > 0) or
      (charindex('(', Sbj12_1) > 0 or charindex('(', Sbj12_2) > 0 or charindex('(', Sbj12_3) > 0) or
      (charindex('(', Sbj13_1) > 0 or charindex('(', Sbj13_2) > 0 or charindex('(', Sbj13_3) > 0) or
      (charindex('(', Sbj14_1) > 0 or charindex('(', Sbj14_2) > 0 or charindex('(', Sbj14_3) > 0) or
      (charindex('(', Sbj15_1) > 0 or charindex('(', Sbj15_2) > 0 or charindex('(', Sbj15_3) > 0) or
      (charindex('(', Sbj16_1) > 0 or charindex('(', Sbj16_2) > 0 or charindex('(', Sbj16_3) > 0) or
      (charindex('(', Sbj17_1) > 0 or charindex('(', Sbj17_2) > 0 or charindex('(', Sbj17_3) > 0) or
      (charindex('(', Sbj18_1) > 0 or charindex('(', Sbj18_2) > 0 or charindex('(', Sbj18_3) > 0) or
      (charindex('(', Sbj19_1) > 0 or charindex('(', Sbj19_2) > 0 or charindex('(', Sbj19_3) > 0) or
      (charindex('(', Sbj20_1) > 0 or charindex('(', Sbj20_2) > 0 or charindex('(', Sbj20_3) > 0) or
      (charindex('(', Sbj21_1) > 0 or charindex('(', Sbj21_2) > 0 or charindex('(', Sbj21_3) > 0) or
      (charindex('(', Sbj22_1) > 0 or charindex('(', Sbj22_2) > 0 or charindex('(', Sbj22_3) > 0) or
      (charindex('(', Sbj23_1) > 0 or charindex('(', Sbj23_2) > 0 or charindex('(', Sbj23_3) > 0) or
      (charindex('(', Sbj24_1) > 0 or charindex('(', Sbj24_2) > 0 or charindex('(', Sbj24_3) > 0) or
      (charindex('(', Sbj25_1) > 0 or charindex('(', Sbj25_2) > 0 or charindex('(', Sbj25_3) > 0) or
      (charindex('(', Sbj26_1) > 0 or charindex('(', Sbj26_2) > 0 or charindex('(', Sbj26_3) > 0) or
      (charindex('(', Sbj27_1) > 0 or charindex('(', Sbj27_2) > 0 or charindex('(', Sbj27_3) > 0))
)

update tblZStudentReport2
set Ind = '各科比重：英文300 中文300 數學300 物理100 化學100 生物100 中史200 地理100 生活與社會50 普通話50 聖經50 電腦認知50' + char(13) + char(10) + 'STEAM教育50'
where form = 3 and Term_EName = 1 and Ind = ''
...
-- ENG 所有卷目缺席問題
select * from tblZStudentReport2
where form in (1,2,3) and Num = '01' and Term_EName = 1
...
-- CHI 所有卷目缺席問題
select * from tblZStudentReport2
where (Term_EName = 1 and ( Sbj08_2 = '　　缺席' and Sbj09_2 = '　　缺席') and Form in (1,2,4,5))

select * from tblZStudentReport2
where (Term_EName = 1 and ( Sbj07_2 = '　　缺席' and Sbj08_2 = '　　缺席') and Form in (3))
...
select * from tblZStudentReport2
where idStudent in (24071) and Term_EName = 1
...
-- Term 2
stpGenerateReport_S123_2014 1, 2
GO
stpGenerateReport_S123_2014 2, 2
GO
stpGenerateReport_S123_2023 3, 2
GO

-- 製作教師輸入評語操行初稿、學生核對稿時方才加入
stpGenerateReport_S123_2014 4, 2
GO
stpGenerateReport_S123_2014 5, 2
GO
--stpGenerateReport_S123_2014 6, 2   # 中六級核對稿程式碼於檔案最尾
--GO
...
-- Term 2 取消 STM 平時分 
select * from tblZStudentReport2
where form in (1,2) and Sbj20_1 <> '' and term = 2
...
-- CHI 所有卷目缺席問題
select * from tblZStudentReport2
where (Term_EName = 2 and ( Sbj08_2 = '　　缺席' and Sbj09_2 = '　　缺席' and Sbj10_2 = '　　缺席' and Sbj11_2 = '　　缺席') and Form in (1,2,4,5))

select * from tblZStudentReport2
where (Term_EName = 2 and ( Sbj07_2 = '　　缺席' and Sbj08_2 = '　　缺席' and Sbj09_2 = '　　缺席' and Sbj10_2 = '　　缺席' and Sbj11_2 = '　　缺席') and Form in (3))
...
-- 處理黃偉立的體育科資料
select * from tblZStudentReport2
where idStudent = 24103 and term = 2

update tblZStudentReport2
set Sbj25_3 = Sbj24_3, Lsn_25 = ''
where idStudent = 24103 and term = 2

-- Term 2 處理中三科目過多，導致欄位 Ind （比重及平合格標示）不能顯示
--update tblZStudentReport2
--set Ind = '各科比重為：英文300 中文300 數學300 物理100 化學100 生物100 歷史200 地理100 生活與社會50 普通話50 聖經50 電腦認知50' + char(13) + char(10) + 'STEAM教育50'
--where form = 3 and Term_EName = 2

--update tblZStudentReport2
--set Ind = Ind + N'　( ) 表示不及格'
--where form = 3 and Term_EName = 2 and idStudent in (
--select idStudent from tblZStudentReport2
--where form = 3 and Term_EName = 2 and (
--      (charindex('(', Sbj01_1) > 0 or charindex('(', Sbj01_2) > 0 or charindex('(', Sbj01_3) > 0) or
--      (charindex('(', Sbj02_1) > 0 or charindex('(', Sbj02_2) > 0 or charindex('(', Sbj02_3) > 0) or
--      (charindex('(', Sbj03_1) > 0 or charindex('(', Sbj03_2) > 0 or charindex('(', Sbj03_3) > 0) or
--      (charindex('(', Sbj04_1) > 0 or charindex('(', Sbj04_2) > 0 or charindex('(', Sbj04_3) > 0) or
...
select * from tblZStudentReport2
where idStudent in (21105, 21082) and Term_EName = 2
...
--	select s.idStudent from tblStudentReportRemark a
--	left join tblStudent s on s.idStudent = a.idStudent
--	where a.term = 2 and a.nameChinese like '受%' and left(s.class,1) in (1,2,3)
--)
...
-- 中六級學生版核對稿
stpGenerateReport_S123_2014 6, 2
GO
