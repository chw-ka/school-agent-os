...
declare  @tuningFactor real

set @term = 2
set @form = 6
set @tuningFactor = 7.45
...
case when
(
	CASE WHEN fps.scoreExam IS NOT null AND sps.scoreExam IS NOT null 
	THEN cast(100.0 as real) * fpw.weightExam * sps.scoreExam / fps.scoreExam ELSE 0.0 END / fpw.weightExam
...
(
	CASE WHEN fps.scoreExam IS NOT null AND sps.scoreExam IS NOT null 
	THEN cast(10000.0 as real) * fpw.weightExam * sps.scoreExam / fps.scoreExam ELSE 0.0 END / fpw.weightExam
)
end
), 
case when sps.flgIgnore is null then 0 else sps.flgIgnore end, 
...
INSERT tblZStudentRank2(idStudent, form, idPaper, flgStandard, section, term, score, flgIgnore, flgAbsent)
SELECT sp.idStudent, sp.form, sp.idSubject, 0, 'E', @term, 
floor(sum(floor(case when score is null then 0.0 else score end / 10.0 + .5) * 10.0 * case when weight is null then 0.0 else weight end) / sum(case when weight is null then 0.0 else weight end)), 0, 0
FROM vwStudentPaper sp
...
FROM tblZStudentRank2 zsr, (
	...
	LEFT JOIN (
		...
		INNER JOIN tblZStudentRank2 zsr ON s.idStudent = zsr.idStudent
		WHERE zsr.score is not null
	) r2 ON r.class = r2.class and r.idPaper = r2.idPaper and r.flgStandard = r2.flgStandard AND r.section = r2.section AND r.term = r2.term and   convert(decimal(7,1), r.score / 100.0)    <  convert(decimal(7,1), r2.score / 100.0)
...
-- 如需要調分，請直接選擇以上所有程式碼，然後執行。其中 TuningFactor 需要自行於程式頂部修改
--或許會有重複紀錄
--delete from tblZStudentRank2
--where idStudent in (select idStudent from tblStudent where left(class,1) = 6) and form = 5

stpCalculateScore_2013 6, 2
GO